...
Suppose we need to find $T(\mathbf{x})$. Since $\mathbf{e}_1,\dots,\mathbf{e}_n$ are linear independent, $\begin{bmatrix}\mathbf{e}_1 & \cdots & \mathbf{e}_n\end{bmatrix}$ has a pivot position at each row and each column. The equation $\begin{bmatrix}\mathbf{e}_1 & \cdots & \mathbf{e}_n\end{bmatrix}\mathbf{t}=\mathbf{x}$ has a unique solution.
