...
Let $r=\opn{rank}A$, then $x_1=\cdots=x_r=0$ for equation $B\bf{x}=(AM)\bf{x}=\bf{0}$, while $x_{r+1},\dots,x_n$ are free variables completely independent of $x_1,\dots,x_r$. Therefore, we can write a basis for $\opn{Nul}(AM)$: $\{\bf{e}_{r+1},\dots,\bf{e}_n\}$. Since elementary matrices are invertible, $M$ is also invertible. Thus, for all $\bf{v}\in \opn{Nul}A$, there exists $\bf{x}=M^{-1}\bf{v}\in\opn{Nul}(AM)$ such that $AM(M^{-1}\bf{v})=AM\bf{x}=\bf{0}$. Conversely, for all $\bf{x}\in\opn{Nul}(AM)$, we have $M\bf{x}\in\opn{Nul}A$. Therefore, left-multiplying each vector in the basis for $\opn{Nul}(AM)$---$\bf{e}_{r+1},\dots,\bf{e}_n$---gives a basis for $\opn{Nul}A$. Moreover, because these vectors are columns in an identity matrix, the basis for $\opn{Nul}A$ is simply the $r+1$-th column to the $n$-th column of $M$.
