...
First, we find an arbitrary basis of $U$, $\{\bf{b}_1,\dots,\bf{b}_m\}$. Let $A=\mat{\bf{b}_1^T \\ \vdots \\ \bf{b}_m^T}$, assuming $\bf{b}_k$ is a column vector. Having row reduced $A$ to its RREF $A'$, we obtain a basis of $U$ where each vector is orthogonal to the others by reading the rows of $A'$. Lastly, we normalize each vector so that they all have a length of $1$. Denote the new basis as $\cal{B}'=\{\bf{b}'_1,\dots,\bf{b}'_m\}$.
...
Let $M=\mat{[\bf{t}_1]_{\cal{B}'} \\ \vdots \\ [\bf{t}_{\dim V}]_{\cal{B}'}}$, where $\{\bf{t}_1,\dots,\bf{t}_{\dim V}\}$ is an arbitrary basis of $V$. Applying the transformation $[\bf{x}]_{\cal{B}'} \mapsto \bf{x}$ on $\opn{Nul}M$ gives the subspace $W$, concluding the proof.
:::
